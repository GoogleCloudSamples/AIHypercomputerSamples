# _TUTORIAL_TITLE_
# Fine-tune Qwen3 on a TPU GKE Cluster

This code sample is intended for AI/ML engineers and demonstrates how to fine-tune the Qwen3 model on a GKE cluster with Cloud TPUs.

Note: This sample doesn’t demonstrate how to prepare a dataset or serve the model in production.

## Architecture

The sample provisions a GKE cluster with a v6e-32 TPU node pool and a c4d-standard-96 CPU node pool. It converts the model format, runs the fine-tuning workload, and converts the trained model back to Hugging Face format.
...
* `1_build_image.sh`: Builds the required Docker image.
* `2_setup_cluster.sh`: Sets up the GKE cluster.
* `3_convert_model.sh`: Converts the model to the required format.
* `4_train_model.sh`: Submits the fine-tuning workload.
* `5_convert_model_hf.sh`: Converts the model back to Hugging Face format.
...
* A Google Cloud project with billing enabled. Running this sample provisions billable Google Cloud resources including:
  ...
  - Compute Engine (Cloud TPU v6e-32 and CPU c4d-standard-96 node pools)
  - Cloud Storage
  - Artifact Registry
...
6. Convert the trained model to Hugging Face format.
   ```bash
