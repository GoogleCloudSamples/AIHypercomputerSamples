...
# Extract the unique benchmark run prefix (e.g., pkb-21e2db86-0) to catch all attempt clusters
RUN_PREFIX=$(echo "${CLUSTER_NAME}" | grep -oE '^pkb-[a-f0-9]+-[0-9]+' || echo "${CLUSTER_NAME}")

# 1. Discover ALL clusters generated by this run (including retries like attempt 1, 2, 3)
echo "Searching for all clusters matching prefix: ${RUN_PREFIX}..."
CLUSTERS_TO_DELETE=()
while IFS= read -r line; do
  [ -n "$line" ] && CLUSTERS_TO_DELETE+=("$line")
done < <(gcloud container clusters list --project="${PROJECT}" --location="${REGION}" --filter="name~'^${RUN_PREFIX}'" --format="value(name)" 2>/dev/null || true)
...
if [ ${#CLUSTERS_TO_DELETE[@]} -gt 0 ]; then
  for TARGET_CLUSTER in "${CLUSTERS_TO_DELETE[@]}"; do
    ...
    # 1a. Cleanly remove XPK workloads/JobSets if they exist
    ...
    # 1b. CRITICAL: Delete node-pools and wait for completion to instantly release TPU reservation
    echo "Deleting all node pools in ${TARGET_CLUSTER} to prevent auto-healing..."
    ACTIVE_OPS=()
    for pool in $(gcloud container node-pools list --cluster="${TARGET_CLUSTER}" --location="${REGION}" --project="${PROJECT}" --format="value(name)" 2>/dev/null); do
      if [ -n "$pool" ]; then
        echo " -> Deleting node pool: $pool"
        OP_ID=$(gcloud container node-pools delete "$pool" \
          --cluster="${TARGET_CLUSTER}" \
          --location="${REGION}" \
          --project="${PROJECT}" \
          --quiet --format="value(name)" 2>/dev/null || true)
        if [ -n "$OP_ID" ]; then
          ACTIVE_OPS+=("$OP_ID")
        fi
      fi
    ...
    # Wait for node pool deletions to finish before triggering cluster deletion
    for op in "${ACTIVE_OPS[@]}"; do
      echo " -> Waiting for nodepool deletion operation: $op..."
      gcloud container operations wait "$op" --location="${REGION}" --project="${PROJECT}" --quiet 2>/dev/null || true
    done

    # 1c. Initiate cluster deletion with retry logic to avoid conflicts with ongoing operations
    echo "Triggering cluster deletion for ${TARGET_CLUSTER}..."
    for attempt in {1..5}; do
      if gcloud container clusters delete "${TARGET_CLUSTER}" --location="${REGION}" --project="${PROJECT}" --quiet --async 2>/dev/null; then
        echo "Successfully scheduled deletion of ${TARGET_CLUSTER}."
        break
      fi
      echo "Cluster deletion blocked by ongoing operation, retrying in 15s (attempt ${attempt}/5)..."
      sleep 15
    done
  done
else
  echo "No active GKE clusters found matching prefix ${RUN_PREFIX}."
fi
