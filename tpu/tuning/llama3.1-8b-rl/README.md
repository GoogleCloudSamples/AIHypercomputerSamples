# _TUTORIAL_TITLE_
# Reinforcement Learning (RL) Fine-tuning for Llama 3.1 8B on TPU

This code sample is intended for AI/ML engineers and demonstrates how to perform Reinforcement Learning (RL) fine-tuning for the Llama 3.1 8B model on Google Cloud TPUs.

Note: This sample doesn’t demonstrate how to prepare a dataset or serve the model in production.

## Architecture

The sample provisions a single Cloud TPU v6e-8 using a TPU VM architecture. The training scripts execute directly on the provisioned TPU VM to run a reinforcement learning fine-tuning workload for the Llama 3.1 8B model.
...
For the complete step-by-step tutorial of how to use this sample, see the official Google Cloud documentation: [Run reinforcement learning training on a TPU VM by using MaxText](https://docs.cloud.google.com/ai-hypercomputer/docs/tutorials/tpu/run-rl-v6e-8-tpu-vm).
