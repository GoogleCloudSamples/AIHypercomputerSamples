...
# [START hypercomputer_tpu_tune_gemma4_e4b_rl_convert_model]
gcluster job submit \
  ...
  sed -i 's/text_base = \"model.language_model\" if vcfg else \"model\"/text_base = \"model.language_model\"/g' /deps/src/maxtext/checkpoint_conversion/utils/param_mapping.py; \\
...
# [END hypercomputer_tpu_tune_gemma4_e4b_rl_convert_model]
...
while [ -z "$POD_NAME" ]; do
  ...
  POD_NAME=$(kubectl get pods -l job-name=gemma4-hf-to-mt-main-job-0 -o jsonpath="{.items[0].metadata.name}" 2>/dev/null || true)
  if [ -z "$POD_NAME" ]; then
    sleep 10
    ATTEMPTS=$((ATTEMPTS+1))
  fi
