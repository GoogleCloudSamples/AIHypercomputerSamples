...
# [START hypercomputer_tpu_tune_gemma4_e4b_rl_convert_hf]
gcluster job submit \
...
# [END hypercomputer_tpu_tune_gemma4_e4b_rl_convert_hf]
...
while [ -z "$POD_NAME" ]; do
  ...
  POD_NAME=$(kubectl get pods -l job-name=gemma4-mt-to-hf-main-job-0 -o jsonpath="{.items[0].metadata.name}" 2>/dev/null || true)
  if [ -z "$POD_NAME" ]; then
    sleep 10
    ATTEMPTS=$((ATTEMPTS+1))
  fi
