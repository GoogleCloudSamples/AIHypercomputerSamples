...
# [START hypercomputer_gpu_tune_gemma3_multihost_gke_create_cluster]
gcloud container clusters create-auto "${CLUSTER_NAME}" \
    --project="${PROJECT_ID}" \
    --location="${CLUSTER_REGION}" \
    --release-channel=rapid
# [END hypercomputer_gpu_tune_gemma3_multihost_gke_create_cluster]
echo "[$(date)] ==================== Cluster created. ===================="

# 2. Get Creds
# [START hypercomputer_gpu_tune_gemma3_multihost_gke_get_creds]
gcloud container clusters get-credentials "${CLUSTER_NAME}" \
    --location="${CLUSTER_REGION}"
# [END hypercomputer_gpu_tune_gemma3_multihost_gke_get_creds]
echo "[$(date)] ==================== Credentials got. ===================="

# 3. Create HF Secret
# [START hypercomputer_gpu_tune_gemma3_multihost_gke_create_secret]
kubectl create secret generic hf-secret \
    --from-literal=hf_api_token="${HF_TOKEN}" \
    --dry-run=client -o yaml | kubectl apply -f -
# [END hypercomputer_gpu_tune_gemma3_multihost_gke_create_secret]
echo "[$(date)] ==================== Secret created. ===================="

# 4. Create Artifact Registry
# [START hypercomputer_gpu_tune_gemma3_multihost_gke_create_repo]
