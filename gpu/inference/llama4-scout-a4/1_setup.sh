...
echo "--------------------------------------------------------"
# [START hypercomputer_gpu_infer_llama4scout_a4_enable_apis]
gcloud services enable \
    artifactregistry.googleapis.com \
    container.googleapis.com \
    storage.googleapis.com \
    --project="${PROJECT_ID}"
# [END hypercomputer_gpu_infer_llama4scout_a4_enable_apis]
...
# [START hypercomputer_gpu_infer_llama4scout_a4_create_bucket]
    gcloud storage buckets create "gs://${GCS_BUCKET}" \
        --project="${PROJECT_ID}" \
        --location="${REGION}"
# [END hypercomputer_gpu_infer_llama4scout_a4_create_bucket]
fi

# 4. Grant Workload Identity access to GCS bucket
# [START hypercomputer_gpu_infer_llama4scout_a4_iam_policy]
gcloud storage buckets add-iam-policy-binding "gs://${GCS_BUCKET}" \
    --member="serviceAccount:${PROJECT_ID}.svc.id.goog[default/default]" \
    --role="roles/storage.objectAdmin" \
    --quiet
# [END hypercomputer_gpu_infer_llama4scout_a4_iam_policy]
...
# [START hypercomputer_gpu_infer_llama4scout_a4_create_ar]
    gcloud artifacts repositories create "${REPO_NAME}" \
        --repository-format=docker \
        --location="${REGION}" \
        --project="${PROJECT_ID}" \
        --description="Docker repository for vLLM images"
# [END hypercomputer_gpu_infer_llama4scout_a4_create_ar]
fi

# 6. Configure Docker authentication
# [START hypercomputer_gpu_infer_llama4scout_a4_auth_docker]
gcloud auth configure-docker "${REGION}-docker.pkg.dev" --quiet
# [END hypercomputer_gpu_infer_llama4scout_a4_auth_docker]
