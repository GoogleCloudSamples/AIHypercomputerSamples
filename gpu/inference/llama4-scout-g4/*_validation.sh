...
echo "=============================================================================="
echo "Running Llama 4 Scout Serving Validation Job on 8x RTX Pro 6000 (G4)..."
...
VALIDATION_PAYLOAD=$(cat <<EOF
{
  "model": "${TARGET_MODEL}",
  "messages": [
    {"role": "user", "content": "Hello Llama 4! Give me a short 3-word technical greeting for G4 GPUs."}
  ],
  "max_tokens": 30,
  "temperature": 0.1
}
...
if echo "$RESPONSE" | grep -q "chat.completion"; then
    ...
    MODEL_OUTPUT=$(kubectl exec -i "${POD_NAME}" -c serving -- python3 -c '
import sys, json
try:
...
' <<< "$RESPONSE")
    ...
    echo "=============================================================================="
    echo "SUCCESS: Llama 4 Scout is operational and ready to serve traffic on RTX Pro 6000 (G4) GPUs!"
else
