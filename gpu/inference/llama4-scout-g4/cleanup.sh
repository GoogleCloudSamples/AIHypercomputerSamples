...
echo "=============================================================================="
echo "STARTING COMPLETE CLEANUP OF LLAMA 4 / G4 (RTX Pro 6000) INFRASTRUCTURE"
...
read -p "Are you sure you want to delete ALL these G4 resources? (y/N) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
if [ -n "${GCS_BUCKET:-}" ] && gcloud storage buckets describe "gs://${GCS_BUCKET}" >/dev/null 2>&1; then
    echo "Cleaning bucket contents and deleting gs://${GCS_BUCKET}..."
    gcloud storage rm --recursive "gs://${GCS_BUCKET}" --quiet 2>/dev/null || true
    echo "✓ Bucket deleted."
else
    echo "✓ Bucket gs://${GCS_BUCKET} does not exist or was not specified."
fi

echo "--------------------------------------------------------"
echo "Step 6: Cleaning Local Workspace Files & Tunnels"
echo "--------------------------------------------------------"
...
echo "All GKE clusters, G4 node pools, Docker artifacts, and GCS buckets removed."
echo "=============================================================================="
